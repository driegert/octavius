...
several chains — as of 2026-08-18 `lilripper:8010` serves the **main chain
primary**, the subagent primary, the vision primary, the reader LLM, and the
...
- **LLM chain (main agent)**: via `OCTAVIUS_LLM_CHAIN`, defaulting to:
  ...
  - second fallback: `lilbuddy:8010/v1/chat/completions` (`gemma4-26b-a4b`) — the only hop on another host, so the only thing keeping this chain alive if lilripper is down. **This hop was silently dead until 2026-08-18**: it named the bare `qwen3.6-35b-a3b`, which a real completion proved is gone from lilbuddy's catalog too (`400 model 'qwen3.6-35b-a3b' not found`, in 2 ms), so that alias now exists on **no** host in the fleet. Re-verify **this hop specifically** after any *lilbuddy* rebuild, not just the lilripper ones — a missing alias 400s instantly, `LLMChainClient` buckets it as an ordinary failover, and `/health` still lists three configured endpoints, so the chain looks like it has a cross-host last resort when it has none.
    - **Do not put a 35B here.** The hop was briefly repointed at `qwen3.6-35b-a3b-mtp-q4-general`; asking lilbuddy to load it produced 25 s of nothing and then **502 on every alias and every port on the box** (`:8010` router, `:8020` bge-m3 embeddings, `:8880` Kokoro TTS all down together) until the services restarted themselves ~2 min later. Caddy stayed up throughout, which is why it was 502 rather than a connection refusal. An A4B is the right size for a 128 GB unified-memory box.
    - `gemma4-26b-a4b` measured on lilbuddy 2026-08-18: 16 s cold load, then 0.5-0.9 s for short replies; **tool calls work** and come back in correct OpenAI shape (`finish_reason: tool_calls`) in ~0.9 s; accepts image input.
    - **It is a thinking model, and it returns reasoning in a separate `reasoning_content` field rather than inline `<think>` tags.** The `<think>` stripper therefore never sees it, and `agent.py`'s `delta.get("content", "")` drops it for free — no code change needed. But it thinks *hard*: 714 reasoning deltas against 30 content deltas on a one-sentence question, ~6.5 s before the first visible token (0.5 s on a trivial prompt). Acceptable for a third fallback that only runs when lilripper is down. **The trap is `max_tokens`:** at `max_tokens=120` this model spent the entire budget on reasoning and returned `finish_reason: length` with an **empty** `content`. Octavius sends no `max_tokens` anywhere today, so it is safe — but anything that starts to must budget for reasoning tokens.
...
- **Subagent LLM chain**: separate routing for delegated subagents via `OCTAVIUS_SUBAGENT_LLM_CHAIN`, defaulting to:
  ...
  - **Keep `:8010`'s consumers on one alias.** Since 2026-08-18 that is nearly everything — main chain primary, subagent primary, vision primary, reader, summary primary — and they all name `qwen3.6-35b-a3b-mtp-general`. If any one of them disagrees, interleaving a document read with a consult thrashes the router between resident models. Same applies to `:8020`, which now carries the main/vision/subagent/summary *fallbacks* on that same alias.
...
- **Embeddings**: bge-m3 chain via `OCTAVIUS_EMBEDDING_CHAIN`, defaulting to:
  ...
  - **`conversations.indexed`** (added 2026-08-10, additive migration) records the
    ...
    complete forever.
- **Vision LLM chain**: image-input turns (Matrix `image_input` frames) via `OCTAVIUS_VISION_LLM_CHAIN`, defaulting to `lilripper:8010` (`qwen3.6-35b-a3b-mtp-general`) with `lilripper:8020` (`qwen3.6-35b-a3b-mtp-general`) as fallback — reordered with the main chain on 2026-08-18, so it now duplicates that chain's first two hops exactly. The `:8010` primary is **behind auth** (see "Configuration and secrets"). The chain stays separate anyway because the main chain has a **third** hop on another host, chosen for *availability* rather than for modality — separate chains are what stop a future "add another fallback so voice survives a lilripper outage" edit to `llm_chain` from silently widening where an image turn can land. That is a guard against a future edit, not a claim about today's catalog: as of 2026-08-18 every lilripper alias on both ports takes images, and so does `gemma4-26b-a4b`, the alias the main chain's third hop now names — meaning **a cross-host vision hop is currently possible** and is the obvious way to close the vision chain's lilripper-only limitation. Check `architecture.input_modalities` before adding one; lilbuddy does still carry text-only aliases (`DEFAULT`, `nemotron-nano-30b`). Separate `LLMChainClient` instance (`vision_llm_client` in `service_clients.py`); see `agent.py`'s `use_vision` routing in `stream_agent_turn`. Vision routing is sticky per thread (`Conversation.has_images`): after the first image the whole thread stays on the vision chain and image content arrays stay in memory; on thread re-attach they re-hydrate from the spool via the `attachments` table when the file still exists. Persisted history/memory only ever see text placeholders.
