...
- **LLM chain (main agent)**: via `OCTAVIUS_LLM_CHAIN`, defaulting to:
  ...
  - second fallback: `lilbuddy:8010/v1/chat/completions` (`qwen3.6-35b-a3b-mtp-q4-general`) — the only hop on another host, so the only thing keeping this chain alive if lilripper is down. **This hop was silently dead until 2026-08-18**: it named the bare `qwen3.6-35b-a3b`, which a real completion proved is gone from lilbuddy's catalog too (`400 model 'qwen3.6-35b-a3b' not found`, in 2 ms). That bare alias now exists on **no** host in the fleet. Repointed at the q4 MTP variant lilbuddy does carry. This is the failure mode CLAUDE.md warns about and it is nearly invisible: a missing alias 400s instantly, `LLMChainClient` treats it as an ordinary failover, and `/health` reports three configured endpoints while only two could ever have answered — so re-verify **this hop specifically** after any lilbuddy rebuild, not just the lilripper ones.
