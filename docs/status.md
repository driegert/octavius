...
- **Every turn was silently running on the third fallback hop (2026-08-26, FIXED).** `~/.config/octavius/env` held a stale token in `OCTAVIUS_LLM_API_KEYS` and no `OCTAVIUS_LR_API_KEY`, while the live token lived in `~/.config/secrets.env` — which the service does not read. Both lilripper hops 401'd (`:8010` sending the stale key, `:8020` sending none because the key was pinned to the `:8010` origin) and every turn landed on lilbuddy `gemma4-26b-a4b`: **28 s to first spoken word vs 3 s**, 172 `401`s in 72 h, `/health` showing `failover_requests == total_requests`. `consult_specialist`, the vision chain, the reader LLM and summaries have no third hop and were failing outright and silently. Three fixes: the key was moved to `OCTAVIUS_LR_API_KEY` in the service env file and its scope widened from `KEYED_8010_ORIGIN` to `KEYED_LR_ORIGINS` (both lilripper ports, since `:8020` had also gone behind auth); the `:8020` entries were repointed from the vanished `qwen3.6-35b-a3b-mtp-general` to `qwen3.8-27b`; and `SummaryClient` gained a per-URL model. The token duplication was then removed outright (2026-08-26): the unit's `ExecStart` sources `~/.config/secrets.env` in a subshell and exports `OCTAVIUS_LR_API_KEY` alone, so only one copy exists fleet-wide. A plain second `EnvironmentFile=` would **not** have worked — that file uses `export FOO=...` lines, which systemd silently ignores. Turn latency is now **1.5 s** end-to-end, helped by thinking-off on the main primary (see below). **Lesson: a chain that is 100 % failing over looks identical to a chain that is working, from everywhere except `/health`.** `scripts/octavius-models check` now catches both the dead-alias and rotated-token cases before a restart.

- **Model routing moved into one file (2026-08-26).** `~/.config/octavius/models.json` holds every LLM role as `{url, model, params?}`; `params` merges into the request body, which is the only route to generation knobs the agent path never builds. Precedence is code defaults < file < env. The headline knob is thinking: `chat_template_kwargs: {"enable_thinking": false}` takes `qwen3.6-35b-a3b-mtp-general` from 2.25 s to **0.22 s** to first content token with tool-calling unchanged. It ran that way on the main primary for two days and was **reverted 2026-08-28** — the latency win was real and tool selection stayed correct, but the answers themselves were less trustworthy, and answer quality is the point of the primary. Every role now runs thinking on; first audio is ~2.2-4.0 s rather than ~1.5-1.8 s. Prefer `reasoning_effort` as a dial over `enable_thinking` as a switch if this is revisited. See CLAUDE.md "Model routing".
